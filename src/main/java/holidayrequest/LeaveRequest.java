...
//			return;
		...
		ProcessInstance processInstance6 = runtimeService.startProcessInstanceByKey("leaveRequest", "businessKey", variables30);
...
//		        });
				...
				if (task.getTaskDefinitionKey().equals("athleteRequestTask")) {
					...
					taskService.claim(task.getId(), processVariables.get("requestAthlete").toString());
					taskService.addCandidateUser(task.getId(), processVariables.get("requestAthlete").toString());
				...
				if (task.getTaskDefinitionKey().equals("trainingApprovalTask")) {
					if (isTrainingReject) {
						...
						taskService.setVariable(task.getId(), "approvalResult", "驳回");
						taskService.setVariableLocal(task.getId(), "approvalResult", "驳回");
						taskService.addComment(task.getId(), processInstance6.getId(), "六天的请假申请，教练员同意，训练科驳回。");
						managementService.executeCommand(new DeleteOthersTaskCmd(processInstance6.getId(), task.getId()));
						taskService.complete(task.getId());
						break;
					} else {
						taskService.setVariable(task.getId(), "approvalResult", "通过");
						taskService.setVariableLocal(task.getId(), "approvalResult", "通过");
						taskService.addComment(task.getId(), processInstance6.getId(), "六天的请假申请，教练员同意，训练科同意。");
						taskService.complete(task.getId());
					}
				}
				if (task.getTaskDefinitionKey().equals("headmasterApprovalTask")) {
					if (isHeadmasterReject) {
						...
						taskService.setVariable(task.getId(), "approvalResult", "驳回");
						taskService.setVariableLocal(task.getId(), "approvalResult", "驳回");
						taskService.addComment(task.getId(), processInstance6.getId(), "六天的请假申请，教练员同意，班主任同意，训练科同意，校长驳回。");
						managementService.executeCommand(new DeleteOthersTaskCmd(processInstance6.getId(), task.getId()));
						taskService.complete(task.getId());
						break;
					} else {
//						taskService.setVariable(task.getId(), "approvalResult", "通过");
//						taskService.setVariableLocal(task.getId(), "approvalResult", "通过");
//						taskService.addComment(task.getId(), processInstance6.getId(), "六天的请假申请，教练员同意，班主任同意，训练科同意，校长同意。");
//						taskService.complete(task.getId());
					}
